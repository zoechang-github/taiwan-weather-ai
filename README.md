...
1. 📱 **雙分欄沙盒（Sandbox）**
   * **上半部**：執行中的台灣天氣查詢 App 預覽。串接**中央氣象署（CWA）開放資料 API**，點下拉選單即時顯示全台 22 縣市的真實天氣（天氣現象、降雨機率、最高/最低溫）。未設定 CWA 金鑰時自動回退到內建示範資料。
...
4. 點右上角 ⚙️ 設定：
   * 貼上 [Gemini API Key](https://aistudio.google.com/app/apikey) → 啟用 AI 協同設計。
   * （選填）貼上 [中央氣象署 CWA 授權碼](https://opendata.cwa.gov.tw/user/authkey) → 顯示**真實天氣**；留空則用示範資料。
